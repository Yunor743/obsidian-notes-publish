...
export const Transformer = {
    ...
    // This function is responsible to transform markdown to html
    getHtmlContent: function (markdownContent) {
        // console.log("MARKDOWN CONTENT")
        // console.log(markdownContent)
        let htmlContent = []
        const sanitizedContent = Transformer.preprocessThreeBackticks(Transformer.preprocessThreeDashes(markdownContent))
    ...
    preprocessThreeDashes: function (content) {
        return content // FIXME this function removed a lot of content
        const indexOfFirst = content.indexOf("---");
        if (indexOfFirst === -1) {
            return content
        }
    ...
    /* SANITIZE MARKDOWN FOR UNKNOW ``` LANGUAGE */
    preprocessThreeBackticks: function (content) {
        const defaultLanguage = "```"
        const forbiddenLanguages = ["sh", "shell"]
        for (let i = 0; i < forbiddenLanguages.length; i++) {
            var forbiddenStr = "```"+forbiddenLanguages[i]
            const firstNdx = content.indexOf(forbiddenStr);
            if (firstNdx === -1) {
                continue
            }
            var byteAfter = content[firstNdx + forbiddenStr.length]
            if (byteAfter != '\n' && byteAfter != ' ') {
                continue
            }
            const before = content.substring(0, firstNdx);
            const after = content.substring(firstNdx + forbiddenStr.length);
            content = before + defaultLanguage + after;
            i = 0
        }
        return content
    },
    ...
    getInternalLinks: function (aFilePath) {
        ...
        const internalLinks = []
        const sanitizedContent = Transformer.preprocessThreeBackticks(Transformer.preprocessThreeDashes(fileContent))
        unified()
            .use(markdown, {gfm: true})
            .use(wikiLinkPlugin, {
                pageResolver: function (pageName) {
